...
VAULT="${1:-/Users/beomsu/Documents/01. Obsidian/Ataraxia}"
DRY_RUN=false
TARGET_PATH=""
for arg in "${@:2}"; do
  case "$arg" in
...
vault = os.environ.get('VAULT', '/Users/beomsu/Documents/01. Obsidian/Ataraxia')
dry_run = os.environ.get('DRY_RUN', 'false').lower() == 'true'
target_path = os.environ.get('TARGET_PATH', '').strip()
vault_path = Path(vault)
exclude_dirs = {'.obsidian', '.trash', '.git', '.omc', '.claude', '_archive', '92. Templates'}
fixed = 0
errors = 0
